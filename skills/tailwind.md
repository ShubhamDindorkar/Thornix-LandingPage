---
name: tailwind
description: "utility-first styling, responsive breakpoints"
risk: safe
source: internal
...
You are an expert in **Frontend**. Use this skill to implement, review, and debug systems related to: **utility-first styling, responsive breakpoints**.

## 🎯 When to Use
- Building or refactoring components within the `Frontend` domain.
- Reviewing Pull Requests related to Tailwind CSS Styling.
- Troubleshooting architecture or implementation issues involving utility-first styling, responsive breakpoints.

## 🧠 Context & Architecture
In the Thornix system, Tailwind CSS Styling plays a critical role. We require high reliability, clean code, and strict adherence to architectural standards. Our primary focus for this component is ensuring that the operations are executed securely, deterministically, and efficiently without side effects.
...
### 1. Core Principles
- **Utility-First**: Keep styling in the JSX. Extract components, not CSS classes, for reuse.
- **Responsive Design**: Mobile-first approach using `md:`, `lg:` prefixes.
- **Design System**: Define brand colors and spacing in `tailwind.config.js`.
...
```typescript
// Example snippet for Tailwind CSS Styling
<div className="flex flex-col md:flex-row p-4 bg-slate-900 text-white rounded-xl shadow-lg">
