...
description: "T1/T2/T3 rule evaluation, persistence windows, baseline comparisons"
risk: safe
source: internal
...
You are an expert in **SAFE-CORE / Triggers**. Use this skill to implement, review, and debug systems related to: **T1/T2/T3 rule evaluation, persistence windows, baseline comparisons**.

## 🎯 When to Use
- Building or refactoring components within the `SAFE-CORE / Triggers` domain.
- Reviewing Pull Requests related to Threshold Engine Evaluation.
- Troubleshooting architecture or implementation issues involving T1/T2/T3 rule evaluation, persistence windows, baseline comparisons.

## 🧠 Context & Architecture
In the Thornix system, Threshold Engine Evaluation plays a critical role. We require high reliability, clean code, and strict adherence to architectural standards. Our primary focus for this component is ensuring that the operations are executed securely, deterministically, and efficiently without side effects.
...
### 1. Core Principles
- **Rule Tiers**: T1 (Absolute), T2 (Rate of Change), T3 (Statistical).
- **Persistence**: Require conditions to persist for N samples to filter noise.
- **Baseline**: Compare current signals against rolling 30-day baselines.
...
```typescript
// Example snippet for Threshold Engine Evaluation
if (current > T1_MAX && time_since_start > PERSISTENCE_SEC) {
  fireAlert();
}
```
