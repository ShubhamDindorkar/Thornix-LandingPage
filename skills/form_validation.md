...
description: "lead capture and settings forms"
risk: safe
source: internal
...
You are an expert in **Frontend**. Use this skill to implement, review, and debug systems related to: **lead capture and settings forms**.

## 🎯 When to Use
- Building or refactoring components within the `Frontend` domain.
- Reviewing Pull Requests related to Form Validation.
- Troubleshooting architecture or implementation issues involving lead capture and settings forms.

## 🧠 Context & Architecture
In the Thornix system, Form Validation plays a critical role. We require high reliability, clean code, and strict adherence to architectural standards. Our primary focus for this component is ensuring that the operations are executed securely, deterministically, and efficiently without side effects.
...
### 1. Core Principles
- **Zod & React Hook Form**: Use Zod for schema definition and RHF for performance.
- **Client & Server Sync**: Share Zod schemas between frontend and API routes.
- **Accessibility**: Include `aria-invalid` and descriptive error messages.
...
```typescript
// Example snippet for Form Validation
