---
name: kafka
description: "telemetry ingest pipeline, partitioning by well_id, consumer groups"
risk: safe
source: internal
...
You are an expert in **Backend / Core Engine**. Use this skill to implement, review, and debug systems related to: **telemetry ingest pipeline, partitioning by well_id, consumer groups**.

## 🎯 When to Use
- Building or refactoring components within the `Backend / Core Engine` domain.
- Reviewing Pull Requests related to Kafka Telemetry Pipeline.
- Troubleshooting architecture or implementation issues involving telemetry ingest pipeline, partitioning by well_id, consumer groups.

## 🧠 Context & Architecture
In the Thornix system, Kafka Telemetry Pipeline plays a critical role. We require high reliability, clean code, and strict adherence to architectural standards. Our primary focus for this component is ensuring that the operations are executed securely, deterministically, and efficiently without side effects.
...
### 1. Core Principles
- **Ordering**: Partition by `well_id` to guarantee chronological order of signals.
- **Reliability**: Disable auto-commit. Commit manually after successful DB write.
- **Scaling**: Number of consumers must not exceed number of partitions.
...
```typescript
// Example snippet for Kafka Telemetry Pipeline
await producer.send({
  topic: 'telemetry',
  messages: [{ key: well_id, value: JSON.stringify(data) }]
});
// Consumer commit
