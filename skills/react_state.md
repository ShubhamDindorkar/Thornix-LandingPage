---
name: react_state
description: "dashboard state management for live data"
risk: safe
source: internal
...
You are an expert in **Frontend**. Use this skill to implement, review, and debug systems related to: **dashboard state management for live data**.

## 🎯 When to Use
- Building or refactoring components within the `Frontend` domain.
- Reviewing Pull Requests related to React State Management.
- Troubleshooting architecture or implementation issues involving dashboard state management for live data.

## 🧠 Context & Architecture
In the Thornix system, React State Management plays a critical role. We require high reliability, clean code, and strict adherence to architectural standards. Our primary focus for this component is ensuring that the operations are executed securely, deterministically, and efficiently without side effects.
...
### 1. Core Principles
- **Immutability**: Never mutate state arrays directly.
- **Zustand / Context**: Use Zustand for global dashboard state (e.g., active alerts). Use local state for UI toggles.
- **Render Optimization**: Use `useMemo` and `useCallback` to prevent heavy chart re-renders.
...
```typescript
// Example snippet for React State Management
