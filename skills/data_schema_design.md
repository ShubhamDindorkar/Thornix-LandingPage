...
description: "append-only tables, hash-chain tamper evidence, provenance fields"
risk: safe
source: internal
...
You are an expert in **Data & Storage**. Use this skill to implement, review, and debug systems related to: **append-only tables, hash-chain tamper evidence, provenance fields**.

## 🎯 When to Use
- Building or refactoring components within the `Data & Storage` domain.
- Reviewing Pull Requests related to Data Schema Design.
- Troubleshooting architecture or implementation issues involving append-only tables, hash-chain tamper evidence, provenance fields.

## 🧠 Context & Architecture
In the Thornix system, Data Schema Design plays a critical role. We require high reliability, clean code, and strict adherence to architectural standards. Our primary focus for this component is ensuring that the operations are executed securely, deterministically, and efficiently without side effects.
...
### 1. Core Principles
- **Append-Only**: Treat telemetry and logs as immutable append-only ledgers.
- **Tamper Evidence**: Use cryptographic hash chaining (hash of previous row) for critical audit trails.
- **Provenance**: Always store `created_by`, `source_system`, and timestamps.
...
```typescript
// Example snippet for Data Schema Design
CREATE TABLE audit_log (
  id SERIAL PRIMARY KEY,
  action TEXT,
  prev_hash TEXT,
  hash TEXT GENERATED ALWAYS AS (md5(action || prev_hash)) STORED
);
```
