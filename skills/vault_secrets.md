---
name: vault_secrets
description: "HashiCorp Vault / AWS Secrets Manager integration, no secrets in code"
risk: safe
source: internal
...
You are an expert in **Security**. Use this skill to implement, review, and debug systems related to: **HashiCorp Vault / AWS Secrets Manager integration, no secrets in code**.

## 🎯 When to Use
- Building or refactoring components within the `Security` domain.
- Reviewing Pull Requests related to Vault & Secrets Management.
- Troubleshooting architecture or implementation issues involving HashiCorp Vault / AWS Secrets Manager integration, no secrets in code.

## 🧠 Context & Architecture
In the Thornix system, Vault & Secrets Management plays a critical role. We require high reliability, clean code, and strict adherence to architectural standards. Our primary focus for this component is ensuring that the operations are executed securely, deterministically, and efficiently without side effects.
...
### 1. Core Principles
- **No Secrets in Code**: Never commit keys, passwords, or tokens.
- **Dynamic Secrets**: Use Vault to generate short-lived database credentials.
- **Runtime Injection**: Inject secrets as environment variables during container startup.
...
```typescript
// Example snippet for Vault & Secrets Management
const dbPassword = process.env.DB_PASSWORD; // Injected by Vault agent
if (!dbPassword) throw new Error('Missing secret');
```
